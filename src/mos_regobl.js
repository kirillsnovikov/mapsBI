  ...
  function drawMap(mapData) {
      ...
      if (curState === self.zoomScale.UD) {
        ...
        if (self.zoomScale.citiesVisible()) {
          mapCollection.citiesTableColumns = [
            ...
            new self.tableColumn({
              name:
                window.title === 'Ком.доход' ? 'TP_PROGNOZ' : 'EMPL_PROGNOZ',
              display: window.title + ', прогноз',
              align: 'right',
              class: 'map-table__cell_type_prc',
              width: 80
      ...
      function getTotalEmpl(data) {
        ...
        keys.forEach(function(key) {
          if (key === 'FAKT') {
            ...
            total['EMPL_PLAN'] = getSum(data, 'EMPL_PLAN');
          } else if (key === 'PRC') {
            if (total.EMPL_PLAN === '0' && total.EMPL_FAKT === '0') {
              total['EMPL_PRC'] = 0;
            } else if (total.EMPL_PLAN === '0' && total.EMPL_FAKT !== '0') {
              total['EMPL_PRC'] = 100;
            } else {
              total['EMPL_PRC'] = Math.round(
                (self.helper.toNumber(total.EMPL_FAKT) /
                  self.helper.toNumber(total.EMPL_PLAN)) *
                  100
              );
            }
          } else if (key === 'PROGNOZ') {
            var sum = self.helper.toNumber(getSum(data, 'EMPL_PROGNOZ_Q'));
            total['EMPL_PROGNOZ'] =
              total.EMPL_PLAN === '0'
                ? 0
                : Math.round(
                    (sum / self.helper.toNumber(total.EMPL_PLAN)) * 100
                  );
          }
        });
        return total;
    ...
    function zoomHandler(d, delta) {
      ...
      function paint(zoomed) {
        ...
        //   paintEntity.call(self, zoomed, 'REGION_PROGNOZ', 'REGION_NAME', 'REGION_FAKT', 'REGION_PLAN', 'REGION_PRC');
        // }
      }
    }
...
//# sourceMappingURL=mosregobl.js.map
