...
export function MapLoader_dolya() {
  ...
  function drawMap(mapData) {
    ...
    self.defineTableColumns = function() {
      ...
      if (curState === self.zoomScale.UD) {
        mapCollection.tableColumns = [
          ...
          new self.tableColumn({
            name: 'ROO_DOLYA',
            display: 'Доля ' + window.title + ', процент',
            align: 'right',
    ...
    self.data.forEach(function(e) {
      ...
      var ctm = Number(
        self.helper.replaceAll(e['CITY_CUSTOMER'], ' ', '') || 0
      );
      maxCustomer = ctm > maxCustomer ? ctm : maxCustomer;
        ...
        self.helper.assign(
          {
            CIRCLE_SIZE:
              (ctm * 100) /
              Number(self.helper.replaceAll(e['REGION_CUSTOMER'], ' ', '') || 1)
          },
          e,
          ['CITY', 'CITY_DOLYA', 'CITY_CUSTOMER', 'LAT', 'LON']
        )
      );
    });
    ...
    function drawCities(code) {
      ...
      cities
        ...
        .style('fill', function(d) {
          return self.color(self.helper.toNumber(d.CITY_DOLYA));
        })
        .attr('stroke-width', 3)
        ...
        .on('click', function(d) {
          ...
          self.createPie(self.getDataPie(d, 'CITY', 'CITY_DOLYA'));
        });
      cities
        .append('text')
        .attr('x', 12)
    ...
    function zoomHandler(d, delta) {
      ...
      function paint(zoomed) {
        ...
        if (self.zoomScale.isZoom(self.zoomScale.UD)) {
          ...
          paintEntity.call(self, zoomed, 'ROO_DOLYA', 'ROO', 'ROO_CUSTOMER');
...
//# sourceMappingURL=drsk_dolya.js.map
